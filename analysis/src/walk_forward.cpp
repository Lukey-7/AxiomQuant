...
WindowPerformance evaluate_window(
    ...
    backtest::BacktestEngine engine(
        universe.slice(slice_begin, end),
        backtest::Portfolio(setup.initial_cash),
        backtest::ExecutionModel(setup.execution),
        engine_cfg
    );
...
std::vector<SmaSweepPoint> sweep_sma_parameters(
    const data::MarketDataUniverse& universe,
    ...
    for (int64_t i = 0; i < static_cast<int64_t>(points.size()); ++i) {
        ...
        backtest::strategies::SmaCrossoverStrategy strategy(ticker, point.fast_period, point.slow_period, 0.95);
...
WalkForwardResult run_sma_walk_forward(
    const data::MarketDataUniverse& universe,
    const std::string& ticker,
    ...
    for (size_t train_begin = 0; train_begin + config.train_bars + config.test_bars <= n; train_begin += config.test_bars) {
        ...
        const auto sweep = sweep_sma_parameters(universe, ticker, config.fast_grid, config.slow_grid,
                                                setup, train_begin, train_end, warmup);
        if (sweep.empty()) {
...
std::string format_walk_forward_report(const WalkForwardResult& r, const std::string& ticker) {
    ...
    ss << "=========================================================================\n";
    ss << "Train " << r.train_bars << " bars -> test " << r.test_bars << " bars, rolled forward by the test length.\n";
    ...
    for (size_t i = 0; i < r.folds.size(); ++i) {
        ...
        ss << std::setw(4) << (i + 1) << "  " << f.test_start << ".." << f.test_end
           << "  " << std::left << std::setw(7) << params << std::right
           << std::setprecision(3) << std::setw(10) << f.in_sample.sharpe_ratio
           << std::setw(11) << f.out_of_sample.sharpe_ratio
    ...
    if (r.folds.empty()) {
        ...
        ss << "  SMA (walk-forward):  return " << std::setw(7) << (r.stitched_strategy.total_return * 100.0)
           ...
           << "   MaxDD " << std::setprecision(2) << std::setw(6) << (r.stitched_strategy.max_drawdown * 100.0) << " %\n";
        ss << "  Buy & hold:          return " << std::setw(7) << (r.stitched_benchmark.total_return * 100.0)
           ...
           << "   MaxDD " << std::setprecision(2) << std::setw(6) << (r.stitched_benchmark.max_drawdown * 100.0) << " %\n";
        ss << std::setprecision(3);
        ss << "  Mean Sharpe: in-sample " << r.mean_in_sample_sharpe
           << "  ->  out-of-sample " << r.mean_out_of_sample_sharpe
           << "  (decay " << (r.mean_in_sample_sharpe - r.mean_out_of_sample_sharpe) << ")\n";
        ss << "  Folds where the strategy out-returned buy & hold: "
           << r.folds_beating_benchmark << " / " << r.folds.size() << "\n";
    }
    ss << "=========================================================================\n";
    return ss.str();
}
