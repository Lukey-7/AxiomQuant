...
std::vector<FrontierPoint> EfficientFrontier::compute_unconstrained_frontier(
    ...
    for (size_t i = 0; i < num_points; ++i) {
        ...
        auto opt = UnconstrainedMarkowitz::target_return_portfolio(expected_returns, cov_matrix, target_r, risk_free_rate);
...
std::vector<FrontierPoint> EfficientFrontier::compute_constrained_frontier(
    ...
    for (size_t i = 0; i < num_points; ++i) {
        double gamma = (i == 0) ? 0.0 : std::pow(10.0, -3.0 + (static_cast<double>(i) / (num_points - 1)) * 5.0);
        auto opt = optimizer.optimize_risk_aversion(expected_returns, cov_matrix, gamma);
        points.push_back({opt.expected_return, opt.volatility, opt.sharpe_ratio, opt.weights});
    }

    // Sort by volatility
    std::sort(points.begin(), points.end(), [](const FrontierPoint& a, const FrontierPoint& b) {
...
std::string EfficientFrontier::render_ascii_frontier(
    ...
    ss << "        " << std::setw(6) << (min_vol * 100.0) << "%"
       << std::string(width > 20 ? width - 20 : 1, ' ')
       << std::setw(6) << (max_vol * 100.0) << "% Volatility\n";
    ss << "Legend: [.] Frontier   [G] Global Min Variance   [T] Max Sharpe Tangency\n\n";

    return ss.str();
}

std::string EfficientFrontier::generate_portfolio_report(
    ...
    ss << "========================================================================================================\n";
    ss << std::left << std::setw(12) << "Asset"
       << std::right << std::setw(18) << "GMV (Uncon)"
       << std::setw(18) << "Tangency (Uncon)"
       << std::setw(18) << "GMV (Long-Only)"
       << std::setw(18) << "Max Sharpe (Long)"
       << std::setw(18) << "Risk Parity"
       << "\n";
    ss << "--------------------------------------------------------------------------------------------------------\n";

    for (size_t i = 0; i < tickers.size(); ++i) {
        ss << std::left << std::setw(12) << tickers[i]
           << std::right << std::setw(17) << (gmv_uncon.weights(i) * 100.0) << "%"
           << std::setw(17) << (tan_uncon.weights(i) * 100.0) << "%"
           << std::setw(17) << (gmv_con.weights(i) * 100.0) << "%"
           << std::setw(17) << (tan_con.weights(i) * 100.0) << "%"
           << std::setw(17) << (risk_parity.weights(i) * 100.0) << "%"
           << "\n";
    }

    ss << "--------------------------------------------------------------------------------------------------------\n";
    ss << std::left << std::setw(12) << "Exp Return"
       << std::right << std::setw(17) << (gmv_uncon.expected_return * 100.0) << "%"
       << std::setw(17) << (tan_uncon.expected_return * 100.0) << "%"
       << std::setw(17) << (gmv_con.expected_return * 100.0) << "%"
       << std::setw(17) << (tan_con.expected_return * 100.0) << "%"
       << std::setw(17) << (risk_parity.expected_return * 100.0) << "%"
       << "\n";

    ss << std::left << std::setw(12) << "Volatility"
       << std::right << std::setw(17) << (gmv_uncon.volatility * 100.0) << "%"
       << std::setw(17) << (tan_uncon.volatility * 100.0) << "%"
       << std::setw(17) << (gmv_con.volatility * 100.0) << "%"
       << std::setw(17) << (tan_con.volatility * 100.0) << "%"
       << std::setw(17) << (risk_parity.volatility * 100.0) << "%"
       << "\n";

    ss << std::left << std::setw(12) << "Sharpe (2%)"
       << std::right << std::setprecision(3)
       << std::setw(18) << gmv_uncon.sharpe_ratio
       << std::setw(18) << tan_uncon.sharpe_ratio
       << std::setw(18) << gmv_con.sharpe_ratio
       << std::setw(18) << tan_con.sharpe_ratio
       << std::setw(18) << risk_parity.sharpe_ratio
       << "\n";
    ss << "========================================================================================================\n";
